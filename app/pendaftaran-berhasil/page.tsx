...
export default function SuksesPage() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      // 2. Kompresi Gambar agar hemat storage dan upload cepat
      if (file.type.startsWith("image/")) {
        const options = { maxSizeMB: 0.2, maxWidthOrHeight: 1280, useWebWorker: true };
        try { 
          file = await imageCompression(file, options); 
        } catch (err) { 
          console.error("Kompresi gagal, menggunakan file asli:", err); 
        }
      }
      ...
      // 5. Ambil Public URL hasil upload
      const { data: publicUrlData } = supabase.storage.from('berkas-ikm').getPublicUrl(filePath);
      const publicUrl = publicUrlData.publicUrl;

      // 6. LOGIKA UPDATE DATABASE - STRATEGI 3 TAHAP (OPTIMAL)
      let updateResult = null;

      // TAHAP 1: Berdasarkan ID UUID (Paling Akurat)
      if (rawId && rawId.length === 36 && rawId !== "undefined") {
        console.log("Mencoba update via ID...");
        const { data } = await supabase
          .from('list_tunggu_peserta')
          .update({ foto: publicUrl })
          .eq('id', rawId)
          .select();
        if (data && data.length > 0) updateResult = data;
      }

      // TAHAP 2: Berdasarkan Nama (Jika ID tidak ada/salah)
      if (!updateResult) {
        console.log("Mencoba update via Nama...");
        const { data } = await supabase
          .from('list_tunggu_peserta')
          .update({ foto: publicUrl })
      ...
      // TAHAP 3: Jurus Pamungkas (Ambil data paling baru secara global)
      // Ini mengatasi masalah jika localStorage kosong total
      if (!updateResult) {
        console.log("Mencoba update via Global Terbaru...");
        const { data, error: finalError } = await supabase
          .from('list_tunggu_peserta')
          .update({ foto: publicUrl })
          .neq('nama_peserta', '') // Klausa WHERE wajib agar tidak di-reject Supabase
          .order('created_at', { ascending: false })
          .limit(1)
          .select();
        
        if (finalError) throw new Error(`Database Reject: ${finalError.message}`);
        updateResult = data;
      }
      ...
      if (updateResult && updateResult.length > 0) {
        ...
        throw new Error("Data pendaftaran tidak ditemukan. Silakan refresh atau daftar ulang.");
      }
